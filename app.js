...
function roundPlayers(roundNumber){
    ...
    .map(p=>{
      ...
      const roundScore=
        S.metric==='human'
          ...
          ?allocations.reduce(
            (sum,a)=>
              sum+Number(a.human_vote),
            0
          )

          :allocations.reduce(
            (sum,a)=>
              sum+Number(a.modelEV),
            0
          );
...
function init(){
  ...
  /*
     If the HTML does not already contain a By Round button,
     create one automatically.
  */

  if(
    $('#viewToggle') &&
    !$('#viewToggle [data-view="round"]')
  ){
    ...
    $('#viewToggle [data-view="round"]').onclick=()=>{
